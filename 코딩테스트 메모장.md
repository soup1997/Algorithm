...
> discount의 최대 길이가 100,000이므로 이중 반복문을 잘못써서 완전 탐색하면 시간 초과가 발생하는 것을 명심해야 한다. 어떻게 최적화 해야할까?
...
> heap에는 우선순위만을 포함하고, queue에는 idx와 우선순위 정보를 모두 저장한다
...
def solution(elements):
    ...
    while length <= len(elements):
        for i in range(len(elements)):
            dq.appendleft(dq.pop()) # 전체 큐를 rotate하는 것 보다 이게 더 빠름
            # dq.rotate(-1) # 반 시계방향으로 1번 회전
    ...
    return len(cases)
```
> python과 비슷하게 cpp에도 set이 존재한다, sort해서 unique하지 말고 한번에 set으로 중복을 처리하자

> python 풀이법의 경우 deque을 한번씩 회전하면서 length 길이 만큼 슬라이싱해서 더함
...
> 재귀로 풀다보니까 시간초과가 발생한다. 단순하게 math의 comb 함수를 이용하자
